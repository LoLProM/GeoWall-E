...
=======
    ...
    private List<string> ParseParameters()
    {
        ...
        while (CurrentToken.Type == TokenType.ColonToken)
        {
            ...
            if (coordinates.Contains(CurrentToken.Text))
            {
                throw new Exception($"! SEMANTIC ERROR : A parameter with the name '{CurrentToken.Text}' already exists insert another parameter name");//Cambiar el error
            }
            coordinates.Add(CurrentToken.Text);
            TokenAhead();
        }
...
=======
    ...
    private GSharpExpression ParseSegment()
    {
        ...
        var parameters = ParseParameters();
        return new GSharpSegmentExpression(segmentKeyword, parameters);
    }
